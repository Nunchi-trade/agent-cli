...
def install_setup_fakes(monkeypatch, paired_wallet=None) -> None:
    ...
    monkeypatch.setattr("cli.web_auth.get_stored_pairing", lambda: paired_wallet)
...
def test_setup_check_warns_on_raw_key_without_pairing(monkeypatch):
    ...
    assert "hl pair connect" in result.output
...
def test_mcp_setup_check_reports_auth_warnings(monkeypatch):
    ...
    assert any("No paired wallet found" in warning for warning in payload["warnings"])
